...
// RecyOs => RecyOs => HubSpotClientProfile.cs
...
public class HubSpotClientProfile : Profile
{
    public HubSpotClientProfile()
    {
    CreateMap<EtablissementClientDto, CompaniesDto>()
        ...
        .ForMember(dest => dest.FoundedYear, opt => opt.MapFrom(src => src.EtablissementFiche.DateDeCreation))
        .ForMember(dest => dest.Country, opt => opt.MapFrom(src => src.PaysFacturation))
        .ForMember(dest => dest.Description, opt => opt.MapFrom(src => src.EntrepriseBase.ObjetSocial))
    ...
    CreateMap<CompaniesDto, EtablissementClientDto>()
