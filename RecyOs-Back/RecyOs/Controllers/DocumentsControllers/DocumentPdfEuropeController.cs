...
[ApiController]
[Route("documents-pdf-europe")]

public class DocumentPdfEuropeController : BaseApiController
{
    private readonly IDocumentPdfEuropeService<DocumentPdfEuropeDto> _documentPdfEuropeService;
    private readonly IFileValidationService _fileValidationService;
    private readonly IConfiguration _configuration;

    public DocumentPdfEuropeController(IDocumentPdfEuropeService<DocumentPdfEuropeDto> documentPdfEuropeService, IFileValidationService fileValidationService, ICurrentContextProvider currentContextProvider, IConfiguration configuration)
    {
        _documentPdfEuropeService = documentPdfEuropeService;
    ...
    /// <param name="etablissementClientEuropeId">Identifiant de la fiche client</param>
    ...
    public async Task<ActionResult> UploadDocumentPdf(IFormFile file, [FromForm] int etablissementClientEuropeId, [FromForm] int typeDocumentPdfId)
    {
            ...
            try
            {
                await _fileValidationService.ValidateEtablissementClientEuropeId(etablissementClientEuropeId);
            }
            ...
            var documentDto = await _documentPdfEuropeService.UploadPdfAsync(file, etablissementClientEuropeId, typeDocumentPdfId);
    ...
    public async Task<ActionResult> DownloadDocumentPdf(int id)
    {
        ...
        var (fileStream, fullPath) = await _documentPdfEuropeService.DownloadPdfAsync(id);
    ...
    {
        var documents = await _documentPdfEuropeService.GetListAsync();
        return Ok(documents);
    }
    ...
    {
        var document = await _documentPdfEuropeService.GetByIdAsync(id);
    ...
    {
        var documents = await _documentPdfEuropeService.GetByClientIdAsync(clientId);
        return Ok(documents);
    }
    ...
    public async Task<ActionResult> UpdateDocumentPdf(IFormFile file, [FromRoute] int id, [FromForm] int? typeDocumentPdfId = null)
    {
        ...
        var documentDto = await _documentPdfEuropeService.UpdatePdfAsync(id, file, typeIdToPass);
        return Ok(documentDto);
    }
    ...
    {
        var result = await _documentPdfEuropeService.DeleteAsync(id);
        if (!result)
        {
            return NotFound();
        }
