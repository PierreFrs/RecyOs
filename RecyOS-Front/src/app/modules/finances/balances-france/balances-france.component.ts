...
import { BalancesFranceService } from './balances-france.service';
import { BalanceFrance, BalanceGridParams, BalancePaginator } from './balances-france.types';
...
@Component({
    selector       : 'finances-balances',
    templateUrl    : './balances-france.component.html',
    styleUrls      : ['./balances-france.component.scss'],
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class BalancesFranceComponent implements OnInit {
    @ViewChild(MatPaginator) private _paginator: MatPaginator;
    @ViewChild(MatSort) private _sort: MatSort;

    balances$: Observable<BalanceFrance[]>;
    ...
    constructor(
        public balancesService: BalancesFranceService,
    ...
    ngOnInit(): void {
        ...
        this.commerciaux$ = this._commerciauxService.fetchCommerciaux().pipe(
            map(commerciaux => {
                // Trier les commerciaux par nom puis prénom
    ...
    ngAfterViewInit(): void {
        // S'abonner aux événements de pagination et de tri
    ...
    private _getBalancesData(): void {
        const params: BalanceGridParams = {
    ...
    // Méthode pour vérifier si le montant dépasse l'encours
    isOverLimit(balance: BalanceFrance): boolean {
        let value = balance.montant * -1 ;
        // On vérifie le dépassement pour les soldes créditeurs (positifs)
        // car ils représentent une dette envers le client
        if (value <= 0) {
            return false;
        }
        return Math.abs(value) > balance.etablissementClient.encoursMax;
    }

    // Méthode pour obtenir le commercial à partir de son ID
